...
{
    public partial class UC_DSPhieuXuat : UserControl
    {
        private void LoadDSPhieuXuat()
        {
            long TongPhieuXuat = 0;
            List<PhieuXuat_DTO> list = PhieuXuat_BUS.TimKiemPhieuXuat(dateTimeFrom.Text.ToString(), dateTimeTO.Text.ToString());
            if (list != null)
            {
            ...
            if (list != null)
            {
                dataGVPhieu.Columns["SoPX"].HeaderText = "Số phiếu xuất";
                dataGVPhieu.Columns["NgayXuat"].HeaderText = "Ngày xuất";
        ...
        private void dataGVPhieu_Click(object sender, EventArgs e)
        {
            ...
            strSoPX = dr_PhieuXuat.Cells["SoPX"].Value.ToString();

            List<PhieuXuat_DTO> list = PhieuXuat_BUS.TimKiemPhieuXuatTG(strSoPX);
            dataGVDSSanPham.DataSource = list;
            if (list != null)
            {
                dataGVDSSanPham.Columns["SoPX"].Visible = false;
                dataGVDSSanPham.Columns["NgayXuat"].Visible = false;
        ...
        private void UC_DSPhieuXuat_Load(object sender, EventArgs e)
        {
            ToolTip tooltip = new ToolTip();
            tooltip.SetToolTip(xoaPhieu, "Xóa phiếu xuất");
            tooltip.SetToolTip(xuatExcel, "Lưu dữ liệu thành excel");
            tooltip.SetToolTip(printPhieu, "In phiếu xuất");
        ...
        private void printPhieu_Click(object sender, EventArgs e)
        {
